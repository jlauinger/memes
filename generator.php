...
if (strpos($_GET["meme"], "..") !== false) die("Main thread threw java.lang.PathTraversalAttemptException");
if (strpos($_GET["meme"], "/") !== false) die("Main thread threw java.lang.PathTraversalAttemptException");
...
class MemeGenerator{
  ...
  private function WorkOnImage($text,$size,$type) {
    ...
    if($type == "upper")
      ...
      $this->PlaceTextOnImage($this->im,$size, $LowerTextX, $TextHeight-35,$this->font, $this->lowerText);
  }
...
if (isset($_GET["custom"]) && isset($_GET["meme"]) && !empty($_GET["meme"]) && file_exists("uploads/".$_GET["meme"])) {
  $meme = "uploads/".$_GET["meme"];
} else if (isset($_GET["meme"]) && array_key_exists($_GET["meme"], $memes)) {
  $meme = $memes[$_GET["meme"]];
} else {
  die("Main thread threw java.lang.UnsupportedMemeException");
}

$obj = new MemeGenerator($meme);
